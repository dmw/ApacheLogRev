...
parseSectionText s = do _ <- string s >> spaces >> char '"'
                        res <- textValue
                        _ <- char '"' >> readEol
                        return res

logSpecHeader :: Parser LRS
logSpecHeader = do specId <- parseSection "log-spec:"
                   _ <- spaces
...
parseParam = do _ <- string "param:" >> spaces >> char ':'
                vn <- identifierValue
                _ <- char ':' >> char '\''
                vl <- singleQuoteValue
                _ <- char '\'' >> readEol
...
parseBuiltIn = do string ":built-in:"
                  return LRSBuiltIn

parseCustom :: Parser LRSType
parseCustom = do string ":custom:"
                 return LRSCustom

logRuleParserParam :: Parser LRSRule
logRuleParserParam = do _ <- spaces
...
logTokenParser = do _ <- spaces >> char '|' >> spaces
                    i <- identifierValue
                    _ <- readEol
                    return i

logTokenizerParser :: Parser [String]
logTokenizerParser = do _ <- string "log-tokenize:"
...
commaDelimVal = do x <- identifierValue
                   _ <- (char '\n' <|> char '\r' <|> char ',')
                   return x

logParseTable :: Parser LRSTable
logParseTable = do _ <- spaces >> string "define-table:" >> spaces
                   g <- identifierValue
                   _ <- char '|'
...
chartTypeBar = do _ <- string "bar-chart"
                  return LRSChartBar

chartTypeLine :: Parser LRSChartType
chartTypeLine = do _ <- string "line-chart"
                   return LRSChartLine

chartTypePie :: Parser LRSChartType
chartTypePie = do _ <- string "pie-chart"
                  return LRSChartPie

logParseChart :: Parser LRSChart
logParseChart = do _ <- spaces >> string "define-chart:" >> spaces
                   g <- identifierValue
                   _ <- char '|'
                   v <- many commaDelimVal
...
logParseStruct = do _ <- spaces >> char '$'
                    i <- identifierValue
                    _ <- char ':'
                    v <- identifierValue
                    _ <- readEol
...
logReportParser = do i <- parseSection "define-report:"
